<template>
	...
	<div class='casePage'>
		...
		<div class='nav'>
			<div class='wrapper'>
				<ul>
					<li v-for='(nav,index) in navs' :key='index' @click="showNav(nav,index)"
						:class="{active: this.currentIndex===index}">{{nav.title}}</li>
				</ul>
			</div>
		</div>
		<div class='product'>
			<div class='wrapper'>
				<router-link v-for='(product,index) in products' :to="product.path" target="_blank" :key='index'
